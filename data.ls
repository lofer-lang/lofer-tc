...
Sum_case: (A: Type) -> (B: Type) -> (C: Type) -> (A -> C) -> (B -> C) -> \
  Sum A B -> C
case A B C fl fr x = x C fl fr

postulate Sum_Fam: (A: Type) -> (B: Type) -> (A -> Type) -> (B -> Type) -> \
  Sum A B -> Type
Fam A B = Sum_case A B Type
...
fst A B p = p A (Sigma_fst_method A B)
...
snd A B = Sigma_elim A B (Sigma_snd_Motive A B) (Sigma_snd_method A B)
