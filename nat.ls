...
map A B f mx = mx (Nat_Node B) (nothing B) (Nat_Node_map_method A B f)
...
case M x f n = pred n M x f

Nat_fold: (M: Type) -> M -> (M -> M) -> Nat -> M
fold M x f = Rec_fold Nat_Node Nat_Node_map M (Maybe_case M M x f)
...
postulate Nat_elim: (M: Nat -> Type) -> M zero -> ((n: Nat) -> M (suc n)) -> \
  (x: Nat) -> M x
elim M mz ms x = pred x (M x) mz ms
...
postulate Nat_ind: (M: Nat -> Type) -> M zero -> ((n: Nat) -> M n -> M (suc n)) -> \
  (x: Nat) -> M x
ind M mz ms = fix ((x: Nat) -> M x) id (Nat_ind_step M mz ms)
